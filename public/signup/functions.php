...
/**
 * Checks if the params of the company signup are all set
 *
 * @param mixed[] Array of parameters from the $_POST
 * @return boolean True if all params are set, otherwise false
 */
function all_company_params($params)
{
  if (isset($params['company_name']) && isset($params['email'])
      && isset($params['password']) && isset($params['password_repeat'])) {
  ...
  if (mysqli_query($connection, $insert_user_sql)) {
    ...
    if (mysqli_query($connection, $insert_student_sql)) {
      ...
      return array(
        ...
        'student_handle' => "{$params['first_name']} {$params['last_name']}"
      );
 ...
 *         and student_id otherwise.
 */
function store_new_company($connection, $params)
{
  $pwd = password_hash($params['password'], PASSWORD_BCRYPT);
  $insert_user_sql = "insert into users (email, password, type_id) ";
  $insert_user_sql .= "values ('{$params['email']}', '$pwd', 3)"; // 3 for company

  if (mysqli_query($connection, $insert_user_sql)) {
    $user_id = mysqli_insert_id($connection);

    $insert_company_sql = "insert into companies (company_name, user_id) values ";
    $insert_company_sql .= "('{$params['company_name']}', $user_id)";

    if (mysqli_query($connection, $insert_company_sql)) {
      $company_id = mysqli_insert_id($connection);

      return array(
        'user_id' => $user_id,
        'company_id' => $company_id,
        'company_handle' => $params['company_name']
      );
